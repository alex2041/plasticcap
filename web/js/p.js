(function($){
	...
	$.fn.simplePagination = function(options)
	{
		...
		return this.each(function()
		{
			...
			// items = $(this).find(settings.pagination_container).children(),
				item_count = parseInt(settings.count),
			...
			function refresh_first(page_number)
			{
				 ...
				 */
				var first_html = '<' + settings.navigation_element + ' href="#" class="' + settings.btn_class + ' ' + settings.html_prefix + '-navigation-first';
				first_html += page_count === 1 || page_number === 1 ? ' ' + settings.btn_active : '';
			...
			function refresh_previous(page_number)
			{
				var previous_page = page_number > 1 ? page_number - 1 : 1,
					previous_html = '<' + settings.navigation_element + ' href="#" class="' + settings.btn_class + ' ' + settings.html_prefix + '-navigation-previous';
				previous_html += page_count === 1 || page_number === 1 ? ' ' + settings.btn_active : '';
			...
			function refresh_next(page_number)
			{
				var next_page = page_number + 1 > page_count ? page_count : page_number + 1,
					next_html = '<' + settings.navigation_element + ' href="#" class="' + settings.btn_class + ' ' + settings.html_prefix + '-navigation-next';
				next_html += page_count === 1 || page_number === page_count ? ' ' + settings.btn_active : '';
			...
			{
				var last_html = '<' + settings.navigation_element + ' href="#" class="' + settings.btn_class + ' ' + settings.html_prefix + '-navigation-last';
				last_html += page_count === 1 || page_number === page_count ? ' ' + settings.btn_active : '';
			...
			function refresh_page_numbers(page_number)
			{
				...
				var half_of_number_of_page_numbers_visable = Math.ceil(number_of_visible_page_numbers / 2) - 1,
					...
					{
						page_number_html = '<' + settings.navigation_element + ' href="#" class="' + settings.btn_class + ' ' + settings.html_prefix + '-navigation-page';
						page_number_html += page_count === 1 || page_number === current_while_page ? ' ' + settings.btn_active : '';
			...
			refresh_simple_pagination(settings.page);
			...
			$(container_id).on('click', settings.navigation_element + '[data-' + settings.html_prefix + '-page-number]', function(e)
			{
				...
				if(window.location.pathname == '/'){
					var uri = 'http://' + window.location.hostname + '/block/get?id=all';
				}else{
					var uri = window.location.href
				}

				var url = updateQueryStringParameter(uri, 'page', page_number);
	...
	$.fn.simplePagination.defaults = {
		...
		btn_active: 'disabled',
		navigation_element: 'button',//button, span, div, et cetera
		items_per_page: 25,
